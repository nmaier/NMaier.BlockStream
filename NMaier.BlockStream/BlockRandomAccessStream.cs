...
public sealed class BlockRandomAccessStream : BlockStream
{
  ...
  public BlockRandomAccessStream(
    Stream wrappedStream,
    bool leaveOpen = false,
    short blockSize = BLOCK_SIZE,
    IBlockCache? cache = null) : this(wrappedStream, new NoneBlockTransformer(), leaveOpen, blockSize, cache)
  {
  }
...
#else
        Extents[currentIndex] = extent with { Length = (short)transformed.Length };
#endif
  ...
  public override int Read(Span<byte> buffer)
  {
    ...
    for (;;) {
      ...
      var blockPos = CurrentPosition % BlockSize;
      // Must not over-read
      var rem = Math.Min(Math.Min(CurrentLength - CurrentPosition, BlockSize - blockPos), buffer.Length);
      if (rem == 0) {
        return read;
      }

      currentBlock.AsSpan((int)blockPos, (int)rem).CopyTo(buffer);
      read += (int)rem;
      CurrentPosition += rem;
  ...
  public override long Seek(long offset, SeekOrigin origin)
  {
    switch (origin) {
      case SeekOrigin.Begin:
        if (offset < 0) {
          ThrowHelpers.ThrowArgumentOutOfRangeException(nameof(offset), offset, "Offset must be positive");
        }
      ...
      case SeekOrigin.End:
        ...
        break;
      default:
        ThrowHelpers.ThrowArgumentOutOfRangeException(nameof(origin), origin, "Invalid Origin");
  ...
  {
    if (value < 0) {
      ThrowHelpers.ThrowArgumentOutOfRangeException(nameof(value), value, "Length must be positive");
    }
...
#endif
  {
    ...
    while (buffer.Length > 0) {
      ...
      if (FillBlock(block)) {
        var blockPos = CurrentPosition % BlockSize;
        var rem = Math.Min(BlockSize - blockPos, buffer.Length);
        buffer.Slice(0, (int)rem).CopyTo(currentBlock.AsSpan((int)blockPos));
        CurrentPosition += rem;
  ...
  private void ReadIndex()
  {
    ...
    var footerLength = ReadInt64LittleEndian(footerLocation);
    if (footerLength < 0) {
      throw new IOException("Block stream has corrupt footer");
    }

    CurrentFooterLength = CurrentLength = ReadInt64LittleEndian(footerLocation.Slice(sizeof(long)));
