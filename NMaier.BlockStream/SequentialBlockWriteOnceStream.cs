...
public sealed class SequentialBlockWriteOnceStream : BlockStream
{
  ...
  public SequentialBlockWriteOnceStream(Stream wrappedStream, bool leaveOpen = false, short blockSize = BLOCK_SIZE) :
    this(wrappedStream, new NoneBlockTransformer(), leaveOpen, blockSize)
  {
  }
  ...
  /// <param name="leaveOpen">Leave the wrapped stream open when disposing this block stream</param>
  /// <param name="blockSize">Block size to use</param>
  public SequentialBlockWriteOnceStream(
