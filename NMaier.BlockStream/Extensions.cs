...
public static class Extensions
{
  ...
  {
    ReadFullBlock(stream, buffer.AsSpan(0, length >= 0 ? length : buffer.Length), length);
  }
  ...
  public static void ReadFullBlock(this Stream stream, ArraySegment<byte> buffer)
  {
    ReadFullBlock(
      stream,
      (buffer.Array ?? throw new ArgumentException("Invalid segment", nameof(buffer))).AsSpan(
        buffer.Offset,
        buffer.Count),
      buffer.Count);
  }
