...
public sealed class BlockStreamTests
{
  private static void BlockStreamSequentialTestRunner(IBlockTransformer transformer)
  {
    ...
    using (var writer = new SequentialBlockWriteOnceStream(ms, transformer)) {
      ...
      Assert.AreEqual(19923020, writer.Position);

      // ReSharper disable AccessToDisposedClosure
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = writer.Seek(0, 0));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => writer.SetLength(0));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = writer.Read(new byte[1]));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = writer.Read(new byte[1], 0, 1));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = writer.Position = 0);
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = writer.Length);
    ...
    using (var reader = new SequentialBlockReadOnlyStream(ms, transformer)) {
      ...
      Assert.AreEqual(19923020, reader.Position);

      // ReSharper disable AccessToDisposedClosure
#if NETFRAMEWORK
      _ = Assert.ThrowsExactly<NotSupportedException>(() => reader.Write(new byte[1], 0, 1));
#else
      _ = Assert.ThrowsExactly<NotSupportedException>(() => reader.Write(new byte[1]));
#endif
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = reader.Seek(0, 0));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => reader.SetLength(0));
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = reader.Position = 0);
      _ = Assert.ThrowsExactly<NotSupportedException>(() => _ = reader.Length);
      // ReSharper restore AccessToDisposedClosure
    }
    ...
    using (var reader = new SequentialBlockReadOnlyStream(ms, transformer)) {
      ...
      reader.ReadFullBlock(new byte[10]);
      reader.ReadFullBlock(new byte[10]);
      Assert.AreEqual(30, reader.Position);
    }

    using var ms2 = new MemoryStream();
    using (var writer = new SequentialBlockWriteOnceStream(ms2, transformer, true)) {
      writer.Write(
      [
        0x1,
        0xff
    ...
    using (var reader = new SequentialBlockReadOnlyStream(ms2, transformer, true)) {
      Span<byte> actual = stackalloc byte[2];
      reader.ReadFullBlock(actual);
      Assert.IsTrue(actual.SequenceEqual(new byte[] { 0x1, 0xff }));
    }
  }
  ...
  private static void BlockStreamWriteOnceTestRunner(IBlockTransformer transformer, [CanBeNull] IBlockCache cache)
  {
    using var ms = new KeepOpenMemoryStream();
    ...
    using (var reader = new BlockReadOnlyStream(ms, transformer, blockSize: 512, cache: cache)) {
      ...
      Assert.AreEqual(0, reader.Seek(0, SeekOrigin.Begin));

      // ReSharper disable AccessToDisposedClosure
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = reader.Seek(-10, SeekOrigin.Begin));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = reader.Seek(-10, SeekOrigin.Current));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = reader.Seek(-reader.Length - 10, SeekOrigin.End));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = reader.Seek(0, (SeekOrigin)(-1)));
      // ReSharper restore AccessToDisposedClosure
      ...
      using var binaryCursorReader = new BinaryReader(reader.CreateCursor(), Encoding.ASCII, true);
      for (var i = 0; i < COUNT; ++i) {
    ...
    using var ms2 = new MemoryStream();
    using (var writer2 = new BlockWriteOnceStream(ms2, transformer, true, 512)) {
      writer2.Write(
      [
        0x1,
        0xff
    ...
    ms2.Seek(0, SeekOrigin.Begin);
    using (var reader2 = new BlockReadOnlyStream(ms2, transformer, true, 512)) {
      Span<byte> actual = stackalloc byte[2];
  ...
  private static void BlockStreamWriterSizeTestRunner(IBlockTransformer transformer, [CanBeNull] IBlockCache cache)
  {
    ...
    using (var writer = new BlockRandomAccessStream(ms, transformer, cache: cache)) {
      ...
      Assert.AreEqual(sizeof(int) * ITEMS, writer.Length);
    ...
    using (var writer = new BlockRandomAccessStream(ms, transformer, cache: cache)) {
      ...
      if (transformer.MayChangeSize) {
        ...
        _ = Assert.ThrowsExactly<IOException>(WriteSome);
    ...
    using var ms2 = new MemoryStream();
    using (var writer2 = new BlockRandomAccessStream(ms2, transformer, true, 512)) {
      writer2.Write(
      [
        0x1,
        0xff
    ...
    using (var reader2 = new BlockRandomAccessStream(ms2, transformer, true, 512)) {
      Span<byte> actual = stackalloc byte[2];
      reader2.ReadFullBlock(actual);
      Assert.IsTrue(actual.SequenceEqual(new byte[] { 0x1, 0xff }));
    }
  ...
  public void BlockRandomAccessStreamTest()
  {
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new BlockRandomAccessStream(new FakeStream(false, false, false)));
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new BlockRandomAccessStream(new FakeStream(false, true, false)));
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new BlockRandomAccessStream(new FakeStream(true, true, false)));
  }
  ...
  {
    BlockStreamTestRunner(new CompositeTransformer(new TransformerTests.TestTransformer(), new NoneBlockTransformer()));
  }
  ...
  {
    BlockStreamTestRunner(new CompositeTransformer(new NoneBlockTransformer(), new TransformerTests.TestTransformer()));
  }
  ...
  public void BlockStreamWriterFileTests()
  {
    ...
    using (var stream = new BlockRandomAccessStream(fs, trans)) {
      // ReSharper disable AccessToDisposedClosure
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = stream.Seek(-10, SeekOrigin.Begin));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = stream.Seek(-10, SeekOrigin.Current));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = stream.Seek(-stream.Length - 10, SeekOrigin.End));
      _ = Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => _ = stream.Seek(0, (SeekOrigin)(-1)));
      // ReSharper restore AccessToDisposedClosure
  ...
  public void BlockStreamWriterNone2Test()
  {
    ...
    using (var writer = new BlockRandomAccessStream(ms)) {
      ...
      _ = writer.Seek(0, SeekOrigin.Begin);
      Assert.AreEqual(0, writer.Position);
      Assert.AreEqual(sizeof(int) * COUNT, writer.Length);
    ...
    using (var writer = new BlockRandomAccessStream(ms)) {
      ...
      _ = writer.Seek(0, SeekOrigin.Begin);
      Assert.AreEqual(0, writer.Position);
      Assert.AreEqual(sizeof(int) * (COUNT + 1), writer.Length);
  ...
  {
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new BlockWriteOnceStream(new FakeStream(false, false, false)));
  }
  ...
  {
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new SequentialBlockReadOnlyStream(new FakeStream(false, false, false)));
  }
  ...
  {
    _ = Assert.ThrowsExactly<ArgumentException>(
      () => _ = new SequentialBlockWriteOnceStream(new FakeStream(false, false, false)));
  }

  private sealed class BlockCache : IBlockCache
  {
    private readonly Dictionary<long, byte[]> items = [];
  ...
  private sealed class FakeStream(bool canRead, bool canSeek, bool canWrite) : Stream
  {
